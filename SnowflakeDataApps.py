  ...
  with col2:
    ...
    with c2tab3: #Apply
      if sc_tb.shape[0]!=0 and alltags.shape[0]!=0: 
        ...
        st.write("Column's datatype:{}".format(str(final4dt).split()[1]))
        cur.execute("Use database {};".format(DB))
        ...
        pol_dict = policyinfo.to_dict('series')
        pol_dict_list =[]
        for x in pol_dict:
          pol_dict_list.append(x)
        pol_dict_list  
      ...
      with c2tab1:
        if sc_tb.shape[0]!=0 and alltags.shape[0]!=0:
          ...
          removeoption = st.radio('',['Remove Mask','Drop Mask'],horizontal=True)
          if removeoption=='Remove Mask':
            st.warning('This option will Remove the mask on all selected columns', icon="⚠️") 
            removemaskon = []
